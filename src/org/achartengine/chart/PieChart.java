  ...
  public void draw(Canvas canvas, int x, int y, int width, int height, Paint paint) {
    ...
    for (int i = 0; i < sLength; i++) {
      ...
      if (seriesRenderer.isHighlighted()) {
        ...
        float translateX = (float) (radius * 0.025 * Math.sin(rAngle));
        float translateY = (float) (radius * 0.025 * Math.cos(rAngle));
        oval.offset(translateX, translateY);
        canvas.drawArc(oval, currentAngle, angle, true, paint);
        oval.offset(-translateX, -translateY);
      } else {
    	 canvas.drawArc(oval, currentAngle, angle, true, paint);
        double rAngle = Math.toRadians(currentAngle + 0.25);
        float yv = (float)(radius * Math.sin(rAngle));
        float xv = (float)(radius * Math.cos(rAngle));
        paint.setColor(Color.WHITE);
        paint.setStrokeWidth(4);
        canvas.drawLine(mCenterX, mCenterY, mCenterX + xv, mCenterY + yv, paint);
      }
      paint.setColor(seriesRenderer.getColor());
    ...
    double rAngle = Math.toRadians(-90);
    float yv = (float)(radius * Math.sin(rAngle));
    float xv = (float)(radius * Math.cos(rAngle));
    paint.setColor(Color.WHITE);
    paint.setStrokeWidth(4);
